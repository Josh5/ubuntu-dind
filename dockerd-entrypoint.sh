...
# Rootless + bind-mounted certs can be unwritable; disable TLS to avoid startup failure.
if is_unprivileged_root && [ -n "${DOCKER_TLS_CERTDIR:-}" ] && [ ! -w "${DOCKER_TLS_CERTDIR}" ]; then
    echo >&2 "warning: DOCKER_TLS_CERTDIR not writable; disabling TLS for rootless mode"
    DOCKER_TLS_CERTDIR=
...
if [ "$1" = 'dockerd' ]; then
    ...
    if [ "$uid" != '0' ]; then
        # if we're not root, we must be trying to run rootless
        if [ ! -x "/home/ubuntu/.docker/bin/dockerd-rootless.sh" ]; then
            echo >&2 "error: attempting to run rootless dockerd but missing '/home/ubuntu/.docker/bin/dockerd-rootless.sh'"
            exit 1
        fi
        ...
        if [ -n "${DOCKER_TLS_CERTDIR:-}" ]; then
            chown -R "$uid:$uid" "$DOCKER_TLS_CERTDIR" 2>/dev/null || true
        fi
        ...
        if [ -n "${DOCKER_TLS_CERTDIR:-}" ] && [ ! -r "${DOCKER_TLS_CERTDIR}/server/key.pem" ]; then
            ...
            DOCKER_TLS_CERTDIR=
            export DOCKER_TLS_CERTDIR
            dockerSocket="${DOCKER_HOST:-unix://${XDG_RUNTIME_DIR}/docker.sock}"
            case "$dockerSocket" in
            unix://*) ;;
            *) dockerSocket="unix://${XDG_RUNTIME_DIR}/docker.sock" ;;
            esac
            set -- dockerd \
    ...
    set -- "$(command -v docker-init || echo /usr/local/bin/docker-init)" -- "$@"
